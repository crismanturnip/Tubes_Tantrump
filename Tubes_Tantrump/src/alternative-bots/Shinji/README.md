...
`Shinji` adalah bot alternatif untuk Robocode Tank Royale yang dibuat sebagai tandingan dari `Kurayami`. Bot ini tetap memakai pendekatan **algoritma greedy**, tetapi heuristic yang digunakan berbeda dari bot utama.
...
Bot ini dibuat untuk menjadi alternatif strategi dalam tugas besar Strategi Algoritma. Fokus utamanya bukan memilih target dengan target score seperti `Kurayami`, melainkan mengambil keputusan greedy berdasarkan:
...
Dengan kata lain, `Shinji` mencoba bertahan lebih aman dulu, lalu melakukan counter attack ketika jarak dan lock target sudah cukup baik.
...
## Perbedaan Dengan Kurayami

| Aspek | Kurayami | Shinji |
| --- | --- | --- |
...
Parameter utama ada di awal file `Shinji.cs`:
...
```powershell
dotnet build Shinji/Shinji.csproj
```
...
`Shinji` adalah bot greedy alternatif yang original dan berbeda dari `Kurayami`. Bot ini cocok dijelaskan sebagai penerapan greedy berbasis heuristic jarak aman, posisi terhadap dinding, energi, dan kualitas target lock.
