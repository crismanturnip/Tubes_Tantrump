...
public class Kurayami : Bot
{
    ...
    private const double RadarLockOvershoot = 1.8;
    private const double EscapeDistance = 140;
    private const int TargetMemoryTurns = 14;
    ...
    private void LockRadar()
    {
        ...
        var radarBearing = NormalizeRelativeAngle(RadarBearingTo(target.X, target.Y));
        RadarTurnRate = Math.Clamp(
            radarBearing * RadarLockOvershoot,
            -MaxRadarTurnRate,
            MaxRadarTurnRate
        );

        if (Math.Abs(radarBearing) < 0.5)
        {
